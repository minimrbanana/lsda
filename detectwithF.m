function detectwithF(rcnn_model, rcnn_feat, imname,orig_img, outname, outdata, outssbox)
th = tic;
% Read image
im = imread(imname);
im_color = imread(orig_img);
% Get boxes
numbox = 256*4;
[boxes, hierarchy,boxesT,priority,segment] = extract_boxes(im, numbox);
boxesS = [boxes max(priority)*ones(size(priority,1),1)-priority];
save(outssbox,'boxesS','segment');

% Extract per region features and scores
[scores, f7] = per_region_features(rcnn_model, rcnn_feat, im_color, boxes);

%[ s_boxes, s_scores ] = remove_box( boxes, scores );
% [Y,I] = sort(scores,2,'descend');
% scoreout=zeros(size(scores,1),size(scores,2));
% for row = 1:size(scores,1)
%    for index = 1:100
%       scoreout(row,I(row,index))=scores(row,I(row,index));
%    end
% end
% scores = scoreout;
...
index = 1;
thresh = 0.9; % percat threshold for nms %%%0.1 former %%% 
...
function [feat, f7] = per_region_features(rcnn_model, rcnn_feat, im, boxes)
